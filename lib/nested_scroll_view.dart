...
/// Signature used by [FixNestedScrollView] for building its header.
///
...
/// to expand.
///
/// [FixNestedScrollView] solves this problem by providing custom
...
///
/// {@tool dartpad}
/// This example shows a [FixNestedScrollView] whose header is the combination of a
...
/// {@end-tool}
///
/// ## [SliverAppBar]s with [FixNestedScrollView]s
///
/// Using a [SliverAppBar] in the outer scroll view, or [headerSliverBuilder],
/// of a [FixNestedScrollView] may require special configurations in order to work
/// as it would if the outer and inner were one single scroll view, like a
/// [CustomScrollView].
///
/// ### Pinned [SliverAppBar]s
///
/// A pinned [SliverAppBar] works in a [FixNestedScrollView] exactly as it would in
...
///
/// This works naturally in a [FixNestedScrollView], as the pinned [SliverAppBar]
...
///
/// In order to float the outer, use [FixNestedScrollView.floatHeaderSlivers]. When
...
///
/// {@tool dartpad}
/// This simple example shows a [FixNestedScrollView] whose header contains a
...
///
/// It is possible with a [FixNestedScrollView] to perform just the snapping
/// animation without floating the app bar in and out. By not using the
/// [FixNestedScrollView.floatHeaderSlivers], the app bar will snap in and out
/// without floating.
///
...
/// implemented in a [FixNestedScrollView]. These widgets take any overlapping
...
/// e.g [FixNestedScrollView.floatHeaderSlivers], the [SliverAppBar] will animate
...
/// Currently, [FixNestedScrollView] does not support simultaneously floating and
...
/// Currently, [FixNestedScrollView] does not support stretching the outer
...
class FixNestedScrollView extends StatefulWidget {
  ...
  /// The widget to show inside the [FixNestedScrollView].
  ///
  /// Typically this will be [TabBarView].
  ///
  /// The [body] is built in a context that provides a [PrimaryScrollController]
  /// that interacts with the [FixNestedScrollView]'s scroll controller. Any
  /// [ListView] or other [Scrollable]-based widget inside the [body] that is
  /// intended to scroll with the [FixNestedScrollView] should therefore not be
  /// given an explicit [ScrollController], instead allowing it to default to
  /// the [PrimaryScrollController] provided by the [FixNestedScrollView].
  final Widget body;

  /// {@macro flutter.widgets.scrollable.dragStartBehavior}
  final DragStartBehavior dragStartBehavior;

  /// Whether or not the [FixNestedScrollView]'s coordinator should prioritize the
  ...
  ///
  /// For sample code showing how to use this method, see the [FixNestedScrollView]
  /// documentation.
  ...
  @override
  FixNestedScrollViewState createState() => FixNestedScrollViewState();
}

/// The [State] for a [FixNestedScrollView].
///
/// The [ScrollController]s, [innerController] and [outerController], of the
/// [FixNestedScrollView]'s children may be accessed through its state. This is
/// useful for obtaining respective scroll positions in the [FixNestedScrollView].
///
/// If you want to access the inner or outer scroll controller of a
/// [FixNestedScrollView], you can get its [FixNestedScrollViewState] by supplying a
/// `GlobalKey<NestedScrollViewState>` to the [FixNestedScrollView.key] parameter).
///
/// {@tool dartpad}
/// [FixNestedScrollViewState] can be obtained using a [GlobalKey].
...
class FixNestedScrollViewState extends State<FixNestedScrollView> {
  ...
  ///    sliver(s) contained in [FixNestedScrollView.headerSliverBuilder].
  ScrollController get innerController => _coordinator!._innerController;

  /// The [ScrollController] provided to the [ScrollView] in
  /// [FixNestedScrollView.headerSliverBuilder].
  ///
  /// This is equivalent to [FixNestedScrollView.controller], if provided.
  ///
  ...
  ///    [ScrollView] contained in [FixNestedScrollView.body].
  ...
  @override
  void didUpdateWidget(FixNestedScrollView oldWidget) {
...
class _InheritedNestedScrollView extends InheritedWidget {
  ...
  final FixNestedScrollViewState state;
...
class _NestedScrollCoordinator
  ...
  final FixNestedScrollViewState _state;
  ...
  _NestedScrollMetrics _getMetrics(
    ...
    print('metrics pixels $pixels');
    // return _NestedScrollMetrics(
    //   minScrollExtent: 56.0,
    //   maxScrollExtent: _outerPosition!.maxScrollExtent +
    //       innerPosition.maxScrollExtent -
    //       56.0 +
    //       extra,
    //   pixels: pixels,
    //   viewportDimension: _outerPosition!.viewportDimension - 56.0,
...
class _NestedScrollPosition extends ScrollPosition
  ...
  //     super.viewportDimension - (debugLabel == 'outer' ? 200.0 : 0.0);
  ...
  bool applyViewportDimension(double viewportDimension) {
    if (debugLabel == 'inner') {
      // viewportDimension = viewportDimension - 56.0;
      print('blub 2 $viewportDimension');
    }
  ...
  double applyClampedDragUpdate(double delta) {
    ...
    print(
        'applyClampedDragUpdate actualNewPixels $actualNewPixels newPixels $newPixels overscroll $overscroll $maxScrollExtent');

    if (debugLabel == 'inner') {
      actualNewPixels = actualNewPixels;
    }
  ...
  double applyFullDragUpdate(double delta) {
    ...
    print(
        'applyFullDragUpdate actualNewPixels $actualNewPixels newPixels $newPixels');
    if (debugLabel == 'inner') {
      actualNewPixels = actualNewPixels;
    }
